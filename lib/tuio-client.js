...
module.exports = function (Tuio) {
  Tuio.Client = Tuio.Model.extend({
    ...
    onListening: function () {
      ...
      console.log(
        "TuioClient listening on: " + address.address + ":" + address.port
      );
    },
    ...
    objectAlive: function (args) {
      ...
      this.aliveObjectList = Tuio._.difference(
        this.aliveObjectList,
        this.newObjectList
      );
    ...
    cursorAlive: function (args) {
      ...
      this.aliveCursorList = Tuio._.difference(
        this.aliveCursorList,
        this.newCursorList
      );
    ...
    cursorRemoved: function (tcur) {
      ...
      if (removeCursor.getCursorId() === this.maxCursorId) {
        ...
        if (Tuio._.size(this.cursorList) > 0) {
          ...
          if (maxCursor && maxCursor?.getCursorId() > this.maxCursorId) {
            this.maxCursorId = maxCursor.getCursorId();
          }
          this.freeCursorList = Tuio._.without(
            this.freeCursorList,
            function (cur) {
    ...
    cursorAdded: function (tcur) {
      ...
      if (cid <= this.maxCursorId && this.freeCursorList.length > 0) {
        ...
        for (var i = 0, max = this.freeCursorList.length; i < max; i++) {
          ...
          if (
            testCursor.getDistanceToPoint(tcur) <
